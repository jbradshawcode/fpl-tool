...
def sample_history_df():
    """Sample history dataframe with various match scenarios (raw API data)."""
    return pd.DataFrame(
        {
...
@pytest.fixture
def processed_history_df(sample_history_df):
    """Sample history dataframe with expected_points already calculated (partially processed)."""
    df = sample_history_df.copy()
    df["expected_points"] = [5.5, 2.5, 7.5, 0.0, 4.0, 6.5, 5.0, 6.0]
    return df


@pytest.fixture
def minimal_history_df():
    """Minimal history with only required columns (raw API data)."""
    return pd.DataFrame(
        {
            "element": [1],
            "round": [1],
